...
% insert_p_queue(Node, PriorityQueue, OutQueue)
...
% build_path(Node, ClosedSet, PathSoFar, Path)
build_path(node(EndState, _ , _ , _, _ ), Nodes, PathSoFar, Path) :-
	del(Nodes, node(EndState, Action, Parent, _ , _), NodesWithoutOne),
	build_path(node(Parent,_ ,_ , _ , _ ),
			   NodesWithoutOne,
			   [Action/EndState|PathSoFar],
			   Path).
